...
## 1. Summary
`T068` is the bounded future implementation candidate that follows `T067`. The slice should keep `/settings` mutation client-owned and presentation-only. The goal is not to add a general-purpose settings backend. The goal is to give the Settings route one honest mutation lane that fits the current diagnostics-first baseline and the existing local locale persistence pattern.
...
The first slice may include one or more of the following if they stay presentation-only:
...
- If the sandbox blocks `test:ui`, record the exact local rerun handoff and expected success signal in this task doc when implementation begins.
