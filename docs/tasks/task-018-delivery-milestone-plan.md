...
- The milestone plan explicitly protects shell closure, feature ownership, and state-boundary stability while sequencing `M3-F1`, `M3-F2`, and gated `M3-F3`.
- Implementation follow-up: `M3-F1` and `M3-F2` are now landed, `../tracking/post-m3-reactivation-guide.md` is now the canonical hold-state and re-entry reference for later post-M3 promotion, Candidate A has since been narrowed and landed as `task-019-server-backed-pre-import-preview.md`, Candidate B and Candidate C remain parked, and `M3-F3` has since moved from gated to reactivated after a later `gate_clear` check.
...
### Milestone 3 — Conditional TSX Presentation Refinement
- Attempt `M3-F3` only after a non-mutating check confirms that the request-builder and active-observation TSX files can be edited safely in the current environment.
- If the sandbox-refresh blocker persists, record `M3-F3` as blocked/deferred and stop the active sequence after `M3-F2`.
- Exit criteria: either `M3-F3` lands as presentation-only refinement or the block is documented without forcing unsafe edits.
...
2. Material 3 follow-up remains limited to bounded visual-only slices, and only `M3-F3` remains unresolved because its TSX validation gate is still blocked.
3. `M3-F3` stays conditional on the TSX sandbox blocker rather than being assumed safe.
...
## 9. Open Questions
1. Whether the TSX sandbox-refresh blocker is still reproducible at execution time remains **확실하지 않음**.
...
## 12. Closure Decision
T018 remains closed as **done** at the planning/documentation level. `M3-F1` and `M3-F2` are landed, `T019` is now landed as the bounded Candidate A follow-up, `M3-F3` now depends on the cleared gate plus the next bounded TSX implementation pass, and the remaining optional backlog stays parked unless the current tracking docs plus `../tracking/post-m3-reactivation-guide.md` justify a later reactivation.
