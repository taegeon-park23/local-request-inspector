...
- A Material 3 adoption plan now exists in `docs/architecture/material-3-adoption-plan.md`, and the initial token/theme rollout plus shell/shared-primitives/request-builder/observation-surface materialization, top-bar role legend, route-role cues, role-specific panel accents, role-specific tab/header treatments, and feature-level list/detail/contextual role strips is now the visual-system baseline for later incremental polish.
