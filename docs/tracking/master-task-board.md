...
- **Verification baseline:** `npm.cmd run lint`, `npm.cmd run typecheck`, and `npm.cmd run test:node` passed on 2026-03-25. `check:app` still reports the known `/app` built-shell gap plus sandbox `spawn EPERM` limits for `build:client` and `test:ui`.
