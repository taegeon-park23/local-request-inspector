...
### Phase AG — Parked Optional Future Work
57. Additional authored-resource tooling beyond `T019` and the current saved-request/mock-rule bundle scope
58. Later write-time migration-engine work if compatibility pressure justifies it
59. Bounded packaging polish only if a delivery milestone identifies a concrete readiness gap
...
- `T052` is landed: Workspace explorer can now create request groups inside existing collections and the request builder exposes explicit save collection/request-group controls, so the next follow-up should narrow one remaining management concern such as request-group rename/delete UI, collection CRUD, or empty-collection placement handling instead of reopening the broader canonical-tree rewrite
