...
- Workspace environment/script helper logic now lives in `server/environment-script-resource-service.js`, mock-rule normalization/list logic now lives in `server/mock-rule-resource-service.js`, authored bundle import planning now lives in `server/resource-bundle-import-service.js`, and execution seed/stage/transport flow logic now lives in `server/execution-flow-service.js`, cutting `server.js` down to 868 lines from the earlier 1686-line baseline.
...
- Node seam tests now cover request-resource mutation, environment/script default reconciliation and listing, the new execution flow service, blocked execution paths, and resource-bundle preview/import under `server/*.test.js`.
