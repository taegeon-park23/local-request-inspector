...
## Current Next Action
No new implementation task is currently promoted automatically after T038. The latest landed client-only slice localizes Environments and Scripts route internals through the shared i18n layer without changing backend contracts, persisted data, or English ARIA defaults. Future localization work should start from `../architecture/client-i18n-foundation.md`, `../tasks/task-037-client-i18n-foundation-and-korean-locale-bootstrap.md`, and `../tasks/task-038-environments-and-scripts-route-localization-pass.md`, then convert one coherent surface at a time. If a later verification step is blocked by the sandbox rather than by repo behavior, ask for a local rerun with exact commands and expected results per `AGENTS.md`.
