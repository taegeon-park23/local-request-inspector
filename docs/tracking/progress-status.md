...
## Current Next Action
No new implementation task is currently promoted automatically after T039. The latest landed client-only slice localizes workspace explorer chrome, request tab shell, request-builder authoring copy, and explorer-owned transfer-status messaging through the shared i18n layer without changing backend contracts, persisted data, or right-hand observation/result semantics. Future localization work should start from `../architecture/client-i18n-foundation.md`, `../tasks/task-037-client-i18n-foundation-and-korean-locale-bootstrap.md`, `../tasks/task-038-environments-and-scripts-route-localization-pass.md`, and `../tasks/task-039-workspace-authoring-localization-pass.md`, then convert one coherent surface at a time. The strongest next translation candidates are the workspace result panel or the Captures / History / Mocks observation routes. If a later verification step is blocked by the sandbox rather than by repo behavior, ask for a local rerun with exact commands and expected results per `AGENTS.md`.
...
30. Open `../tasks/task-037-client-i18n-foundation-and-korean-locale-bootstrap.md`, `../tasks/task-038-environments-and-scripts-route-localization-pass.md`, and `../architecture/client-i18n-foundation.md` before proposing future localization work so later translation passes start from the shared provider, locale persistence, and first-slice proof-of-use boundaries
31. Treat Environments and Scripts as the canonical first route-internal localization pattern before translating broader authoring or observation surfaces, so later slices do not reopen runtime-payload or broad ARIA localization in the same task
