...
- T036 button, badge, and radius visual hierarchy refinement: **done**
- T037 client i18n foundation and Korean locale bootstrap: **done**
- T038 Environments and Scripts route localization pass: **done**
- T039 Workspace authoring localization pass: **done**
- T040 Workspace result panel localization pass: **done**
- T041 Captures observation route localization pass: **done**
- T042 History observation route localization pass: **done**
- T043 shell density and collapsible navigation refinement: **done**
- T044 single-panel route tabs layout: **done**
- T045 Mocks route localization pass: **done**
- T046 shell and management generated ARIA localization pass: **done**
...
## Current Next Action
No new implementation task is currently promoted automatically after T046. The latest landed client-only slice localizes shell route-panel regions plus management/workspace-explorer generated ARIA and action labels through the shared i18n layer while keeping request-builder generated ARIA labels, backend payloads, and runtime-owned status tokens unchanged. Future localization work should start from `../architecture/client-i18n-foundation.md` and narrow one cross-route concern at a time rather than reopening another broad top-level route translation theme. The strongest localization-adjacent follow-up is request-builder generated ARIA-label localization, but it should stay separate from runtime-token translation and broad app-wide ARIA-contract work. If a later verification step is blocked by the sandbox rather than by repo behavior, ask for a local rerun with exact commands and expected results per `AGENTS.md`.
...
- `npm.cmd run typecheck` passed again in this sandbox after `T037` landed on 2026-03-23.
- `npm.cmd run lint:client` passed again in this sandbox after `T037` landed on 2026-03-23.
...
- Direct sandbox `npm.cmd run test:ui` reruns still fail before Vitest transform work with `sandbox_esbuild_transform_blocked` / `spawn EPERM`.
...
- `../tasks/task-040-workspace-result-panel-localization-pass.md` is landed: workspace result-panel tabs, observation header copy, response/console/tests/execution-info section copy, and client-owned fallback text now localize through the shared i18n layer, while runtime-owned status tokens and broader observation routes remain deferred.
- `../tasks/task-041-captures-observation-route-localization-pass.md` is landed: the Captures route sidebar, filter labels, detail/timeline chrome, replay-bridge actions, and client-owned fallback text now localize through the shared i18n layer, while runtime-owned status tokens remain deferred from translation.
- `../tasks/task-042-history-observation-route-localization-pass.md` is landed: the History route sidebar, filter labels, detail/timeline chrome, replay-bridge actions, result-tab labels, and client-owned fallback text now localize through the shared i18n layer, while runtime-owned status tokens remain deferred from translation.
- `../tasks/task-045-mocks-route-localization-pass.md` is landed: the Mocks route sidebar/detail/contextual chrome, route-local empty states, authored-rule action labels, detail-tab labels, and helper-generated authored-rule summaries now localize through the shared i18n layer, while backend payloads and runtime-owned status tokens remain unchanged.
- `../tasks/task-046-shell-and-management-generated-aria-localization-pass.md` is landed: shell route-panel regions plus Environments, Scripts, and Workspace explorer generated ARIA/action labels now localize through the shared i18n layer, while request-builder generated ARIA labels and runtime-owned status tokens remain deferred from translation.
- `../tasks/task-043-shell-density-and-collapsible-navigation-refinement.md` is landed: the shell top bar now targets 42px height, runtime status no longer uses the old card wrapper, navigation can collapse into an icon-only rail without losing accessible route names, and top-level shell separation now relies on shadow-box depth rather than line dividers.
- `../tasks/task-044-single-panel-route-tabs-layout.md` is landed: top-level routes now render explorer, main surface, and contextual detail through one route-local tab strip, so each tab can take the full shell content region without resetting underlying feature state.
- Direct sandbox `npm.cmd run test:ui` reruns are still expected to fail before Vitest transform work with `sandbox_esbuild_transform_blocked` / `spawn EPERM`; the current local verification handoff for the repo state is `npm.cmd run test:ui` with expected result that all tests pass, with the current suite inventory at 11 test files and 63 tests.
...
- Top-bar environment selection, pre-run unresolved-feedback tiering, explorer/readability copy propagation, top-level script attachment/reference semantics, settings mutation, and environment/script/settings import-export expansion remain deferred beyond `T030`.
- Use `environment-follow-up-lane-comparison.md` before proposing further environment work so the next task starts from one compared lane instead of a broad environment umbrella.
- Use `resolved-preview-sub-lane-comparison.md` after `environment-follow-up-lane-comparison.md` before proposing further resolved-preview work so future scope starts from one chosen observation lane rather than from blended feedback/copy themes.
- Use `request-environment-resolution-summary-contract.md` after `resolved-preview-sub-lane-comparison.md` before proposing resolved-preview implementation so the next slice starts from one bounded DTO/UI contract.
...
30. Open `../tasks/task-037-client-i18n-foundation-and-korean-locale-bootstrap.md`, `../tasks/task-038-environments-and-scripts-route-localization-pass.md`, `../tasks/task-039-workspace-authoring-localization-pass.md`, `../tasks/task-040-workspace-result-panel-localization-pass.md`, `../tasks/task-041-captures-observation-route-localization-pass.md`, `../tasks/task-042-history-observation-route-localization-pass.md`, `../tasks/task-045-mocks-route-localization-pass.md`, `../tasks/task-046-shell-and-management-generated-aria-localization-pass.md`, and `../architecture/client-i18n-foundation.md` before proposing future localization work so later translation passes start from the shared provider, locale persistence, and landed bounded reference slices.
31. Treat Environments and Scripts as the canonical first route-internal localization pattern, Workspace authoring as the canonical authoring-chrome pattern, Workspace result panel as the canonical bounded workspace observation-panel pattern, Captures, History, and Mocks as the current full observation-route patterns, and shell/management/workspace-explorer generated labels as the current bounded cross-route ARIA pattern before translating broader concerns, so later slices do not reopen runtime-payload or broad ARIA localization in the same task.
