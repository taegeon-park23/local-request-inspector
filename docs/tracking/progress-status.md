...
## Current Next Action
No new implementation task is currently promoted automatically after T041. The latest landed client-only slice localizes the Captures route sidebar, filter labels, detail/timeline chrome, replay-bridge actions, and client-owned fallback text through the shared i18n layer without changing backend contracts, persisted data, or runtime-owned status token semantics. Future localization work should start from `../architecture/client-i18n-foundation.md`, `../tasks/task-037-client-i18n-foundation-and-korean-locale-bootstrap.md`, `../tasks/task-038-environments-and-scripts-route-localization-pass.md`, `../tasks/task-039-workspace-authoring-localization-pass.md`, `../tasks/task-040-workspace-result-panel-localization-pass.md`, and `../tasks/task-041-captures-observation-route-localization-pass.md`, then convert one coherent observation route at a time. The strongest next translation candidates are the History or Mocks observation routes. If a later verification step is blocked by the sandbox rather than by repo behavior, ask for a local rerun with exact commands and expected results per `AGENTS.md`.
...
31. Treat Environments and Scripts as the canonical first route-internal localization pattern, Workspace authoring as the canonical authoring-chrome pattern, Workspace result panel as the canonical bounded workspace observation-panel pattern, and Captures as the canonical first full observation-route pattern before translating broader observation routes, so later slices do not reopen runtime-payload or broad ARIA localization in the same task.
