...
- `T029` confirms that the next environment-focused implementation slice is request-level selector plus server-owned run-time resolution, and that this should be treated as request-builder follow-up rather than as Candidate A transfer promotion or top-bar selector work.
...
- Keep `../tasks/task-029-request-environment-selection-and-resolution-plan.md` closed as the bounded request-builder follow-up plan for environment selection/resolution, not as a trigger to promote top-bar selector work or authored-resource transfer expansion.
