...
- `../tasks/task-034-post-t033-resolution-summary-contract.md` is landed: the future resolved-preview lane now has one shared execution/history contract, so later work can stay inside one bounded `Execution Info` slice rather than widening into a full preview surface.
...
- `T033` confirms that the strongest future resolved-preview sub-lane is post-run bounded resolution summary, not pre-run feedback tiering or explorer/readability copy.
- `T034` confirms that this future narrow lane should be implemented through one shared execution/history `EnvironmentResolutionSummary` contract rather than through a new tab or a full resolved-request inspector.
...
- Keep `../tasks/task-033-post-t032-resolved-preview-sub-lane-comparison.md` closed as the current narrowing note inside the resolved-preview lane, not as an automatic promotion of result/history resolution-summary work.
- Keep `../tasks/task-034-post-t033-resolution-summary-contract.md` closed as the current contract-definition note for that narrowed lane, not as an automatic promotion of implementation.
