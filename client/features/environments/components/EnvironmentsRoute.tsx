...
export function EnvironmentsRoute() {
  ...
  const patchActiveDraft = (updater: (current: EnvironmentInput) => EnvironmentInput) => {
    setDraft((current) => updater(current.id === activeDraft.id ? current : activeDraft));
  };

  const updateVariableDraft = (
    rowIndex: number,
    updater: (row: EnvironmentVariableInput) => EnvironmentVariableInput,
  ) => {
    patchActiveDraft((current) => ({
      ...current,
      variables: current.variables.map((item, itemIndex) => (itemIndex === rowIndex ? updater(item) : item)),
    }));
  };

  const removeVariableDraft = (rowIndex: number) => {
    patchActiveDraft((current) => ({
      ...current,
      variables: current.variables.filter((_, itemIndex) => itemIndex !== rowIndex),
    }));
  };
      ...
      main={(
        ...
        {listQuery.isPending && !listQuery.data ? <div className="request-work-surface request-work-surface--empty"><EmptyStateCallout title={t('environmentsRoute.empty.loadingDetail.title')} description={t('environmentsRoute.empty.loadingDetail.description')} /></div> : detailQuery.isPending && !detailQuery.data && !isCreatingDraft ? <div className="request-work-surface request-work-surface--empty"><EmptyStateCallout title={t('environmentsRoute.empty.loadingPersistedDetail.title')} description={t('environmentsRoute.empty.loadingPersistedDetail.description')} /></div> : detailQuery.isError && !isCreatingDraft ? <div className="request-work-surface request-work-surface--empty"><EmptyStateCallout title={t('environmentsRoute.empty.degraded.title')} description={`${t('environmentsRoute.empty.degraded.fallbackDescription')} ${detailDegradedReason}`} /></div> : !isCreatingDraft && !detailQuery.data ? <div className="request-work-surface request-work-surface--empty"><EmptyStateCallout title={t('environmentsRoute.empty.noSelection.title')} description={t('environmentsRoute.empty.noSelection.description')} /></div> : (
          <div className="environments-detail">
            ...
            <DetailViewerSection icon="params" title={t('environmentsRoute.detail.variablesCard.title')} description={t('environmentsRoute.detail.variablesCard.description')} className="workspace-surface-card" actions={<button type="button" className="workspace-button workspace-button--secondary" onClick={() => patchActiveDraft((current) => ({ ...current, variables: [...current.variables, createVariableDraft()] }))}><IconLabel icon="add">{t('environmentsRoute.detail.variablesCard.addAction')}</IconLabel></button>}>
              <div className="request-row-editor-list">{activeDraft.variables.map((row, index) => <div key={row.id ?? `environment-row-${index}`} className="environment-row-editor" data-row-kind={row.isSecret ? 'secret' : 'plain'}><div className="environment-row-editor__primary"><label className="request-field"><span>{t('environmentsRoute.detail.variablesCard.labels.key')}</span><input aria-label={t('environmentsRoute.detail.variablesCard.ariaLabels.key', { index: index + 1 })} type="text" value={row.key} onChange={(event) => updateVariableDraft(index, (item) => ({ ...item, key: event.currentTarget.value }))} /></label><label className="request-field"><span>{t('environmentsRoute.detail.variablesCard.labels.valueType')}</span><select aria-label={t('environmentsRoute.detail.variablesCard.ariaLabels.valueType', { index: index + 1 })} value={row.valueType} onChange={(event) => updateVariableDraft(index, (item) => ({ ...item, valueType: event.currentTarget.value as EnvironmentValueType }))}>{valueTypeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label className="request-field"><span>{t('environmentsRoute.detail.variablesCard.labels.description')}</span><input aria-label={t('environmentsRoute.detail.variablesCard.ariaLabels.description', { index: index + 1 })} type="text" value={row.description} onChange={(event) => updateVariableDraft(index, (item) => ({ ...item, description: event.currentTarget.value }))} /></label></div><div className="environment-row-editor__secondary"><div className="environment-row-editor__toggles"><label className="request-field--inline-toggle environment-row-editor__toggle"><span>{t('environmentsRoute.detail.variablesCard.labels.enabled')}</span><input aria-label={t('environmentsRoute.detail.variablesCard.ariaLabels.enabled', { index: index + 1 })} type="checkbox" checked={row.isEnabled} onChange={(event) => updateVariableDraft(index, (item) => ({ ...item, isEnabled: event.currentTarget.checked }))} /></label><label className="request-field--inline-toggle environment-row-editor__toggle"><span>{t('environmentsRoute.detail.variablesCard.labels.secret')}</span><input aria-label={t('environmentsRoute.detail.variablesCard.ariaLabels.secret', { index: index + 1 })} type="checkbox" checked={row.isSecret} onChange={(event) => updateVariableDraft(index, (item) => ({ ...item, isSecret: event.currentTarget.checked, value: '', replacementValue: '', clearStoredValue: false }))} /></label></div><button type="button" className="workspace-button workspace-button--ghost environment-row-editor__remove" aria-label={t('environmentsRoute.detail.variablesCard.ariaLabels.removeVariable', { index: index + 1 })} onClick={() => removeVariableDraft(index)}>{t('environmentsRoute.detail.variablesCard.removeVariable')}</button></div><div className="environment-row-editor__content">{row.isSecret ? <div className="environment-secret-editor"><div className="environment-secret-editor__header"><p className="environment-secret-editor__status">{t('environmentsRoute.detail.variablesCard.secretStatus', { status: row.hasStoredValue && row.clearStoredValue !== true ? t('environmentsRoute.detail.variablesCard.secretStatusAvailable') : t('environmentsRoute.detail.variablesCard.secretStatusEmpty') })}</p></div><div className="environment-secret-editor__controls"><label className="request-field environment-secret-editor__field"><span>{t('environmentsRoute.detail.variablesCard.labels.secretReplacementValue')}</span><input aria-label={t('environmentsRoute.detail.variablesCard.ariaLabels.secretReplacementValue', { index: index + 1 })} type="password" value={row.replacementValue ?? ''} onChange={(event) => updateVariableDraft(index, (item) => ({ ...item, replacementValue: event.currentTarget.value, clearStoredValue: false }))} /></label><button type="button" className="workspace-button workspace-button--ghost environment-secret-editor__clear" aria-label={t('environmentsRoute.detail.variablesCard.ariaLabels.clearStoredSecret', { index: index + 1 })} onClick={() => updateVariableDraft(index, (item) => ({ ...item, replacementValue: '', clearStoredValue: true, hasStoredValue: false }))}>{t('environmentsRoute.detail.variablesCard.clearStoredSecret')}</button></div></div> : <label className="request-field request-field--wide environment-row-editor__value"><span>{t('environmentsRoute.detail.variablesCard.labels.variableValue')}</span><input aria-label={t('environmentsRoute.detail.variablesCard.ariaLabels.variableValue', { index: index + 1 })} type="text" value={row.value ?? ''} onChange={(event) => updateVariableDraft(index, (item) => ({ ...item, value: event.currentTarget.value }))} /></label>}</div></div>)}</div>
            </DetailViewerSection>
          </div>
        )}
        </section>
