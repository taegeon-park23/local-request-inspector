    ...
    <RoutePanelTabsLayout
      ...
      explorer={(
        ...
        <div className="scripts-explorer">
          <FloatingExplorerHeader
            eyebrow={t('scriptsRoute.sidebar.eyebrow')}
            title={t('scriptsRoute.sidebar.title')}
            summary={t('scriptsRoute.sidebar.summary')}
            actions={(
          ...
          <div className="scripts-filter-grid">
            ...
            <label className="request-field request-field--compact">
              ...
              <select aria-label={t('scriptsRoute.sidebar.stageFilterLabel')} value={stageFilter} onChange={(event) => setStageFilter(event.currentTarget.value as ScriptStageFilter)}>
          ...
          {sortedItems.length > 0 ? <ul className="scripts-list" aria-label={t('scriptsRoute.sidebar.listAriaLabel')}>{sortedItems.map((script) => <li key={script.id}><button type="button" className={script.id === effectiveSelectedId && !isCreatingDraft ? 'workspace-request workspace-request--selected' : 'workspace-request'} aria-label={t('scriptsRoute.sidebar.openScriptAction', { name: script.name })} onClick={() => { setIsCreatingDraft(false); setSelectedScriptId(script.id); }}><span className="workspace-request__header"><span className="workspace-request__title" title={script.name}>{script.name}</span><span className="workspace-request__badges"><span className="workspace-chip">{getScriptTypeLabel(script.scriptType, t)}</span></span></span><span className="workspace-request__meta" title={script.description || t('scriptsRoute.list.noDescription')}>{script.description || t('scriptsRoute.list.noDescription')}</span><span className="workspace-request__meta" title={script.sourcePreview || t('scriptsRoute.list.emptySource')}>{script.sourcePreview || t('scriptsRoute.list.emptySource')}</span></button></li>)}</ul> : null}
        </div>
        </section>
      )}
      main={(
